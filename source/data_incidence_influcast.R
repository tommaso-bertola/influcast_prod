...
library(readr)
library(reshape2)
incidence <- function(season = NULL, n_week = NULL, italian_population, reg_pop, consolidate = FALSE, signal_type = "ARI") {
    ...
    source("source/acquisitor_epidemiological.R", local = TRUE)
    df <- influcast_data_acquisitor(n_week, season, signal_type)
