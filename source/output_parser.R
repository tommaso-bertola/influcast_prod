...
raw_incidence_regional <- data.frame()
for (file in files) {
    tmp <- read.csv(file) %>%
    ...
    raw_incidence_regional <- rbind(raw_incidence_regional, tmp)
}
...
write.table(total_table,
    paste0("uploading_predictions/", current_year_week, ".csv"),
    sep = ",",
    row.names = FALSE,
    col.names = TRUE,
    quote = FALSE
...
writeLines(current_year_week, file_conn)
close(file_conn)
...
plot_regional <- regional %>%
    ...
    facet_wrap(~region) +
    theme_light() +
    labs(y = "incidence", x = "weeks ahead", title = paste("Regional prediction", current_year_week, "(dashed line)"))
...
plot_national <- national %>%
    ...
    labs(title = paste("prediction for Italy at", current_year_week, "(dashed line)"), x = "Weeks ahead", y = "ILI incidence")



ggsave(paste0("uploading_predictions/", current_year_week, "_national.png"), plot_national, width = 10, height = 6, dpi = 300)
ggsave(paste0("uploading_predictions/", current_year_week, "_regional.png"), plot_regional, width = 10, height = 6, dpi = 300)
