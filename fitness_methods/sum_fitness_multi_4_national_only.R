...
fitness_ <- function(weekly_inc_vir, data_inc, fitness_tolerance, params) {
    weekly_inc <- weekly_inc_vir$national_inc
    weekly_perc <- weekly_inc_vir$percent_strains
    data_inc <- data_inc$per_thousand$incidence_nat
    vir_inc <- params$virus_percentage

    residuals_df <- abs(weekly_inc - data_inc) / (data_inc + 0.001) # to ensure no division by 0 has to be performed
    res <- 0
    merit_factor <- 1#c(rep(10, 21)) # arbitrarily defined weights for each age group
    ...
    for (col_index in seq_len(ncol(residuals_df))) {
        col_tolerance <- ifelse(residuals_df[, col_index] < fitness_tolerance, 0, residuals_df[, col_index] * merit_factor)
